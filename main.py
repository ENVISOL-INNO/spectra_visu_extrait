...
class FigureSpectra(SpectroVisualization):
    ...
    def plot_all_samples_vs_others(self, add_bands=False) -> dict[str, go.Figure]:
        ...
        for sample_name in self.data.keys():
            ...
            dict_fig[sample_name] = fig
        print("end plot all samples vs others")
        return dict_fig     
...
class SdkApi:
    ...
    def __init__(self, client_):
        """
        This class handles getting/sending data to the server
        """
        self._client: CeFormsClient = client_
        self.project: TriasProject | None = None

    # Problème ici :
    def set_project(self, pid):
        self.project: TriasProject = TriasProject.from_pid(self._client, pid)
...
class DataUpload:
    ...
    def __init__(
        ...
        pid: str,
        path_dir: str,
        sdk_client: CeFormsClient,
        dict_sample_drilling_name: dict[str, str] | None = None,
    ):
        """
        This class does any local file manipulation and uses SdkApi to send data to the server
        """
        self.pid: str = pid
        self.path: str = path_dir
        self.create_temp_path()
        self._client: CeFormsClient = sdk_client
        # commenter les deux lignes ci dessous accélère le process
        # self.sdk = SdkApi(self._client)
        # self.sdk.set_project(pid)
        self.dict_sample_drilling_name: dict[str, str] | None = dict_sample_drilling_name

    def create_temp_path(self):
        if os.path.isdir(self.path):
            self._temp_path = self.path
        else:
            self._temp_path = os.path.split(self.path)[0]
        if not os.path.exists(os.path.join(self._temp_path, "temp")):
            os.makedirs(os.path.join(self._temp_path, "temp"))
        self._temp_path = os.path.join(self._temp_path, "temp")
        return
    ...
    # def construct_spectrum(self, path):
    #     print("construct_spectrum from path:", path)
    #     spectra_object = GET_READER(path)
    #     return spectra_object.make_json()

class BasicFileUpload(DataUpload):
    def __init__(self, project: TriasProject):
        super().__init__(project.get_id(), "", project.client, None)
